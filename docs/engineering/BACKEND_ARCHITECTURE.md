...
The reviewed LA County artifact and an unnumbered, non-activating staging proposal exist, but shared import and activation remain disabled. The proposal stores immutable dataset/boundary/geometry evidence, verifies byte and feature-level provenance, and leaves active rows, current pointers, bounds, live terms, and live relationships unchanged. It may be exercised only on a sentinel-marked disposable target until the canonical fresh/upgrade gate and CTO migration ordering are complete. A later activation must replace approved terms/relationships and derive bounds from the approved County boundary atomically. Do not activate broad LA from bbox membership or edit an already-applied migration.
