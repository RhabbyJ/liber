...
`market_display_geometry_versions` contains a simplified public-safe 393-feature orientation bundle: one County outline, 88 legal-city outlines, and 304 approximate ZCTA outlines. `GET /api/markets/:slug/boundaries?v=:sha256` serves only `kind`, `slug`, `label`, and geometry with immutable caching; it exposes no database UUIDs or source-administration fields. Full selected-area geometry remains separately versioned and authoritative for filtering and map fit.
