import { conversationRouteParamsSchema, markConversationReadSchema } from "@liber/validators";
import type { NextRequest } from "next/server";
import { messagingErrorResponse, parseMessagingJson, privateMessagingJson } from "../../../../../server/messaging/http";
import { markConversationRead } from "../../../../../server/messaging/service";
...
export async function POST(
  ...
  try {
    ...
    const body = await parseMessagingJson(request, markConversationReadSchema);
    return privateMessagingJson(await markConversationRead({ conversationId, ...body }));
  } catch (error) {
